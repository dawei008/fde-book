...
**eval 跑得快是 inner loop 的隐藏前提。**eval-v0 才 10 条，每次跑 30 秒。但是 eval-v1 有 200 条，第一次跑下来 11 分钟。CI 一次 11 分钟，意味着我每次 push 之后要等 11 分钟才能知道有没有 regression——这会直接把 inner loop 从 15 分钟拉到 25 分钟以上。第 6 周末我做了两件事让它降到 4 分钟：评估调用走 Bedrock 的 Flex 服务等级（吞吐量上限高、价格半价），且只对修改过的 prompt 路径跑全量、其他路径走采样。这种工程优化看起来不起眼，但它是 dev velocity 真实的瓶颈。
