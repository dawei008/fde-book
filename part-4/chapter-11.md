...
> "假设我的某个 IAM role 配错了，给了 `bedrock:*` on `*`——这不是不可能，第二期 BU 多了之后权限管理会松。这种情况下，我能不能用网络层挡住，让它**即使配错也调不到 GPT-OSS / Opus 4.7 这些我们没批准的模型**？"
...
加上这份 policy 之后，**即使 Lambda 的 IAM role 给了 `bedrock:InvokeModel` on `*`**，调 Opus 4.7 仍然会被拒——AWS 错误信息里会明确写 `because no VPC endpoint policy allows`，归因清楚。这就是顾建国想要的"配错也兜底"。
...
| Opus 4.7 | ✅ 允许 | ❌ 不允许 | `AccessDeniedException ... because no VPC endpoint policy allows the bedrock:InvokeModel action` |
