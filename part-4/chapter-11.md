...
```
  ...
  3. 工程师登录后, Identity Center 通过 sso:GetRoleCredentials
     (内部走 STS) 发临时凭证 (1 小时过期)
...
跨境数据这一刀也得在这一节落下。合昇业务覆盖东南亚，工单里会出现越南、印尼、马来西亚客户的联系人信息。东南亚几个国家有数据本地化要求（印尼 PP 71/2019、越南网络安全法），我们和顾建国第 9 章已经达成共识：**客户数据不出 ap-southeast-1**。落地：Bedrock 走 ap-southeast-1 的 VPC endpoint；应用日志和 Bedrock Logging 的 S3 桶都在 ap-southeast-1，bucket policy 拒绝跨 region GetObject；CloudTrail 是 multi-region trail 但落地桶在 ap-southeast-1。Cross-region inference profile（4.6 模型走的）是个暗坑——它会把 prompt 跨到 us-east-1 处理。我们和顾建国确认过：合昇这一期只用 4.5 系列（不跨区），4.6 留到下一期评估。

跨境这一刀建议每个 FDE 项目第 2 周就和客户法务 + 安全主管对齐，不要拖到上线前。

---

## 11.6 网络边界：VPC endpoint policy 是另一道门

11.2-11.5 都是 IAM 维度的护栏——人是谁、能干什么、留下什么记录。但 IAM 不是合规客户唯一会问的事。顾建国第 8 周的安全 review 上甩出来一个具体问题：

> "假设我的某个 IAM role 配错了，给了 `bedrock:*` on `*`——这不是不可能，第二期 BU 多了之后权限管理会松。这种情况下，我能不能用网络层挡住，让它**即使配错也调不到 GPT-OSS / Opus 4.6 这些我们没批准的模型**？"

答案是能。Bedrock VPC endpoint 上挂一份 endpoint policy，**和 IAM 完全独立**——endpoint policy 拒绝的请求 IAM 也救不回来。这是合规客户最在乎的"独立审计层"——网络管理员和 IAM 管理员通常是不同团队，policy 也通常是不同团队 review。

合昇第 8 周加上去的那份 endpoint policy 大致这样（节选）：
...
```json
{
  ...
  "Resource": [
    "arn:aws:bedrock:ap-southeast-1:*:inference-profile/apac.anthropic.claude-haiku-4-5-20251001-v1:0",
    "arn:aws:bedrock:ap-southeast-1::foundation-model/anthropic.claude-haiku-4-5-20251001-v1:0"
  ]
}
```

**两条 ARN 都要列**——这是踩过的真实坑。Bedrock 在做权限检查时既看 inference profile ARN（你调的那个 `apac.*`），也看背后的 foundation model ARN（流量真到达的 `anthropic.*`）。只允许 inference profile 不够，会被 foundation model 那一关拦下来。

加上这份 policy 之后，**即使 Lambda 的 IAM role 给了 `bedrock:InvokeModel` on `*`**，调 Opus 4.6 仍然会被拒——AWS 错误信息里会明确写 `because no VPC endpoint policy allows`，归因清楚。这就是顾建国想要的"配错也兜底"。

我把这一套真跑了一遍：建一个无 NAT 私有子网的 Lambda、加 Bedrock VPC endpoint、写上述 policy，让 Lambda 用一个**故意宽松的 IAM** 调两次：

| 调用 | IAM 是否允许 | endpoint policy 是否允许 | 实际结果 |
|---|---|---|---|
| Haiku 4.5 | ✅ 允许 | ✅ 允许 | 成功，response 正常返回 |
| Opus 4.6 | ✅ 允许 | ❌ 不允许 | `AccessDeniedException ... because no VPC endpoint policy allows the bedrock:InvokeModel action` |

第二次的 AWS 原始错误信息里 `because no VPC endpoint policy allows` 这句话是 demo 最值钱的一行——AWS 自己把拒绝归因到 endpoint policy 而不是 IAM，给安全审计直接的证据链。

完整代码在仓库 `demos/ch11-vpc/`，跑通约 30 分钟、成本 < $0.05（VPC interface endpoint $0.01/h 是唯一可量化成本）。一个真实的踩坑：Lambda 在 VPC 内删除后 ENI 会异步清理 1-3 分钟，要给 teardown 留时间或写 force-delete。

> **生产警告**：demo 故意把 Lambda IAM role 配宽松（`bedrock:*` on `*`）是为了让网络层成为**唯一变量**——这样 Opus 被拒就只可能是 endpoint policy 的功劳。**生产中绝对不能这样配 IAM**。生产里 IAM 和 endpoint policy 应该**双层都严**——IAM 限 modelId，endpoint policy 也限 modelId，两层任何一层挂了另一层都救得回来。这是"deep defense"的本意。

---

## 11.7 Guardrails：实时拦住 Agent 自己
...
## 11.8 把 11.2-11.7 串起来：合昇这一期的安全架构
...
[← 上一章：Scaffolding 与开发循环](../chapter-10/) · [下一 Part：PoC → 生产 →](../../part-5/intro/)
