...
完整 14 工具的 schema、错误码、上限值是合昇内部材料的合成品；本书 demo 抽其中 2 个最具示范意义的工具实跑——`query_tickets`（read，演示 schema 严格 + SQL 白名单守卫）和 `lookup_alarm_code`（演示 dry_run + 结构化错误）。代码在仓库 `demos/ch14-agent/`，部署到 AgentCore Runtime + Strands agent + Lambda backend，5 条示范问题跑通。完整 14 工具的扩展形态留给读者按这两个范例外推。
...
[← Part VI 导读](../intro/) · [上一章：监控与 Guardrails](../../part-5/chapter-13/) · [下一章：MCP 与企业集成 →](../chapter-15/)
