...
完整 14 工具的 schema、错误码、上限值是合昇内部材料的合成品；本书 demo 抽其中 2 个最具示范意义的工具实跑——`query_tickets`（read，演示 schema 严格 + SQL 白名单守卫）和 `lookup_alarm_code`（演示 dry_run + 结构化错误）。代码在仓库 `demos/ch14-agent/`，部署到 AgentCore Runtime + Strands agent + Lambda backend，5 条示范问题跑通约 1 分钟、成本约 $0.20。完整 14 工具的扩展形态留给读者按这两个范例外推。
