...
这意味着**理论上 LLM 驱动型 FDE 的 Fix Forward 更容易做到，数据驱动型 FDE 的 Fix Forward 更需要前期争取权限**。但实际上要看客户合规等级——金融、医疗、政企客户的应用层配置中心也会被严格管控，权限优势会被拉平。如果你做的是这类客户的数据驱动型项目，第一周拿 staging 写权限的优先级要更高，否则你的 Fix Forward 几乎不可能落地。
...
[← 上一章：三条铁律](../chapter-02/) · [下一 Part：客户发现 →](../../part-2/intro/)
