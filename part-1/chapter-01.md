...
- **成本测算**：把候选方案各自的 token 用量、API 单价、QPS 算出来。例子：客户要私有部署 70B 模型，你算出他的 QPS 一个月用托管 API 只要私有部署月运维成本的五分之一。
- **延迟实测**：在客户的真实网络条件下测端到端延迟，告诉客户哪条路线 P50 在 1 秒以下、哪条 3 秒。
...
- Conikeec, *The FDE Playbook: A Practitioner's Field Manual* (2025, Substack) — 描述了类似的项目阶段
