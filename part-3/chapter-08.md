...
```json
{"id": "T-2025-Q4-0817",
 "ticket": "主轴异响 + 扭矩报警，无外部电源问题",
 "expected_team": "机械组",
 "expected_fault_type": "主轴/传动"}
```

模型实际输出："机械组 / 主轴"。我的评分逻辑是字符串完全相等：`predicted == expected`。"主轴" ≠ "主轴/传动"，判错。

但**业务上这两个是一回事**——客户的派工流程里"主轴"和"主轴/传动"是同一个分类。我的评估集错把字符串差异当成业务差异，所以四个模型都被卡在 40%。
...
第二个，**评估集没有给"等价表述"留空间**。这条样本如果我写 `expected_fault_type: ["主轴/传动", "主轴", "传动"]`——任何一个匹配都算对——四个模型立刻都到 100%。
...
这是 Ch6 6.3 节那个伏笔的真实闭环。仓库 `demos/ch8-eval/` 起一个 Lambda evaluator，注册到 AgentCore Evaluations，在 TRACE 粒度上对 Ch6 同一组工单分诊预测重新评分。两轮对比的实测结果：
...
完整代码：`demos/ch8-eval/`。注意 AgentCore Evaluations 自 2025-12 进入 preview，boto3 接口仍是 preview 期形态，几个真实坑（OTel trace_id 必须 32-hex、`evaluationReferenceInputs[].context.spanContext` 必填、CreateEvaluator 不支持 upsert）写在了 demo README 里。
...
合昇案例第一期最终会有大约 300-400 条评估集（业务边界明确，比典型 300-500 略小）。这个量级是**够用**的——更大不是更好，关键是覆盖度和评分逻辑的合理性。
...
[← 上一章：RAG / 微调 / Agent 决策树](../chapter-07/) · [下一 Part：工程化落地 →](../../part-4/intro/)
