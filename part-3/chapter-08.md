...
完整代码：`demos/ch8-eval/`。注意 AgentCore Evaluations 的 boto3 接口是 preview 期形态，几个真实坑（OTel trace_id 必须 32-hex、`evaluationReferenceInputs[].context.spanContext` 必填、CreateEvaluator 不支持 upsert）写在了 demo README 里。
