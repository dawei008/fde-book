...
Bedrock 提供三种 evaluation job：Model Evaluation（评估单模型）、Knowledge Base Evaluation（评估 RAG）、Agent Evaluation（评估 agent 多步推理）。

控制台版本最适合 PoC——10 分钟跑通一个 baseline，不写代码。流程：

1. 把 jsonl 评估集传到 S3
2. Bedrock 控制台 → Inference and Assessment → Evaluations → Create job
3. 选 evaluation type（model / KB / agent）
4. 选 evaluator：built-in（关键词、相似度）/ LLM-as-judge / human
5. 跑完看 report——总分、分维度分数、失败样本 CSV、trace 链路

控制台版的局限是不接 CI。Scaffolding 阶段进入正式开发后，建议切到代码版——用 deepeval、promptfoo 这类开源框架，跑在 GitHub Actions 或客户的 CI 系统里。

具体 API 和控制台入口随产品迭代，使用前以 docs.aws.amazon.com 为准。
