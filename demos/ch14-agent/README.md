...
>
> ⚠️ **Gateway teardown**：如果你的账号也被 SCP 拦着 DeleteGateway，`make down` 会保留 gateway 不删（避免无限 retry）——空 gateway 闲置约 $0.01/天的索引存储成本。手工清理：让账号管理员或 root user 跑 `aws bedrock-agentcore-control delete-gateway --gateway-identifier <id> --region us-east-1`。`make verify-down` 会显式提示这种 orphan 状态，不计入 leftover。
